...
- `app/views/rinf_prepended_views/issues/show.html.erb` — новый шаблон; контракт (единственный `h2`, `div.issue.details`, все `call_hook`, `#update`, `#context-menu`, guard-ы `hidden_attribute?`) соблюдён и покрыт живым e2e-чеклистом.
...
- Пустые поля скрыты по умолчанию, раскрываются per-группа («Показать пустые (N)»).
